...
const App: React.FC = () => {
  ...
  const save = useCallback(() => { const j = exportScene(); const b = new Blob([j], { type: 'application/json' }); const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = 'cad-' + new Date().toISOString().slice(0, 10) + '.json'; a.click(); URL.revokeObjectURL(u); }, [exportScene]);
  const open = useCallback(() => finp.current?.click(), []);
  const onFile = useCallback((ev: React.ChangeEvent<HTMLInputElement>) => { const f = ev.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = () => { if (typeof r.result === 'string') importScene(r.result); }; r.readAsText(f); if (finp.current) finp.current.value = ''; }, [importScene]);
    ...
    <div style={S.app}>
      ...
      {ui && <>
        <div style={S.bbar}>
          ...
          <button onClick={() => sel && executeErase(state.selectedId!)} style={{ ...AB, backgroundColor: '#e94560' }} disabled={!sel}>Del</button>
        </div>
        <div style={S.status}>
