...
export function useCADEngine() {
  ...
  const handlePointerDown = (clientX: number, clientY: number) => {
    ...
    if (currentTool === 'select') {
      const hit = objects.find((o) => {
        return o.points.some((p) => Math.abs(p.x - pt.x) < 5 && Math.abs(p.y - pt.y) < 5);
      });
  ...
  // Mount listeners explicitly to the global window scope for Canvas3D attachment
  (window as any).cadDown = handlePointerDown;
